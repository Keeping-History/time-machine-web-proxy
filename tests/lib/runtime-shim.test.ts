...
//
// Same strategy as XHR: replace the prototype method with a spy BEFORE
// reinstalling the shim so the shim captures our spy as `_origLoc*`.
// Calling the patched method then invokes shim → spy, letting us assert on
// the rewritten URL without actually triggering jsdom navigation.

describe("location.assign patch", () => {
	it("rewrites an absolute URL before calling the original assign", () => {
		const captured: string[] = [];
		const realAssign = Location.prototype.assign;
		// biome-ignore lint/suspicious/noExplicitAny: test spy
		(Location.prototype as any).assign = function (url: string) {
			captured.push(url);
		};
		installShim();

		location.assign("http://www.aol.com/");

		Location.prototype.assign = realAssign;
		installShim();

		expect(captured[0]).toBe(`/web/${TS}im_/http://www.aol.com/`);
	});

	it("rewrites a relative URL resolved against the original page", () => {
		const captured: string[] = [];
		const realAssign = Location.prototype.assign;
		// biome-ignore lint/suspicious/noExplicitAny: test spy
		(Location.prototype as any).assign = function (url: string) {
		...
		location.assign("/redirect/target");

		Location.prototype.assign = realAssign;
		installShim();

		expect(captured[0]).toBe(`/web/${TS}im_/http://www.example.com/redirect/target`);
	});
});

describe("location.replace patch", () => {
	it("rewrites an absolute URL before calling the original replace", () => {
		const captured: string[] = [];
		const realReplace = Location.prototype.replace;
		// biome-ignore lint/suspicious/noExplicitAny: test spy
		(Location.prototype as any).replace = function (url: string) {
			captured.push(url);
		};
		installShim();

		location.replace("http://www.aol.com/favicon.ico");

		Location.prototype.replace = realReplace;
		installShim();

		expect(captured[0]).toBe(`/web/${TS}im_/http://www.aol.com/favicon.ico`);
	});
	...
	it("rewrites a relative URL resolved against the original page", () => {
		...
		// ORIG_URL = http://www.example.com/news/story → ../other resolves to /other/page
		expect(captured[0]).toBe(`/web/${TS}im_/http://www.example.com/other/page`);
	});
});

describe("location.href setter patch", () => {
	it("rewrites the value before passing to the original href setter", () => {
		const captured: string[] = [];
		const origDesc = Object.getOwnPropertyDescriptor(Location.prototype, "href");
		if (!origDesc?.set || !origDesc.configurable) {
			// jsdom doesn't expose a configurable href setter — skip
			return;
		}
		...
		Object.defineProperty(Location.prototype, "href", origDesc);
		installShim();
