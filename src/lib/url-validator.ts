...
const matchHostname = (targetHost: string, whitelistHosts: string[]): boolean =>
	whitelistHosts.some((pattern) => {
...
export const isHostnameWhitelisted = (hostname: string, whitelistHosts: string[]): boolean => {
	if (whitelistHosts.includes("*")) return true;
	if (whitelistHosts.length === 0) return false;
	return matchHostname(hostname, whitelistHosts);
};
...
export const isHostWhitelisted = (targetUrl: string, whitelistHosts: string[]): boolean => {
	...
	try {
		...
		return matchHostname(targetHost, whitelistHosts);
	} catch {
		return false;
	}
};
